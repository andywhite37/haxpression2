...
abstract AnnotatedExprGroup<V, A>(AnnotatedExprGroupImpl<V, A>) from AnnotatedExprGroupImpl<V, A> to AnnotatedExprGroupImpl<V, A> {
  ...
  public static function parseMap<V, N, A>(
    ...
    return map.tuples().traverseValidation(function(keyValue : Tuple<String, String>) {
      ...
      return ExprParser.parse(exprString, parserOptions).map(expr -> Tuple.of(key, expr)).toVNel();
    }, Nel.semigroup())
  ...
  public static function parseFallbackMap<V, N, A>(
    ...
    return parseMap(coalesceMap, parserOptions);
  }
  ...
  public function render(valueToString: V -> String) : String {
    return this.foldLeftWithKeys(function(acc : Array<String>, key : String, value: AnnotatedExpr<V, A>) : Array<String> {
      return acc.append('$key: ${ExprRenderer.render(value.expr, valueToString)}');
    }, [])
    .join("\n");
  }
}
